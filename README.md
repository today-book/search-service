...
![재랭킹 과정](images/flowchart_rerank.png)
...
재랭킹 단계에서는 벡터 검색으로 조회된 후보 문서들을 대상으로 **벡터 유사도 점수와 감정 점수를 결합한 최종 점수**를 계산한다. <br>
...
- 재랭킹은 사용자 감정과 맥락을 반영하여 결과 순서를 조정하는 역할을 담당한다.
...
- 검색 및 재랭킹 로직은 특정 LLM에 종속되지 않도록 설계
...
- emotions 파라미터는 감정 기반 검색 및 재랭킹에 사용되는 명시적 감정 신호이다.
